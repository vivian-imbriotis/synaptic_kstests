...
expected_residual_sd = sqrt(INTER_RESIDUAL**2 + WITHIN**2)

expected_overall_sd = sqrt(INTER_FIXED^2 + INTER_RESIDUAL**2 + WITHIN**2)
...
for (i in 1:N){
  ...
  dat <- gen_paired_data(treatment_effect = ATE, interneuron_sd = INTER_FIXED, residual_interneuron_sd = INTER_RESIDUAL,
                           within_neuron_sd = WITHIN, n_neurons = 20)
...
values <- c(expected_overall_sd,expected_overall_sd,0,ATE,INTER_FIXED,expected_residual_sd)
